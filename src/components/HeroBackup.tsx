...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = (e?: any) => {
      ...
      // Güvenli scroll değeri al
      let scrollY = window.scrollY
      if (e && typeof e === 'object') {
        if ('scroll' in e && typeof e.scroll === 'number') {
          scrollY = e.scroll
        }
      }
      ...
      const videoScrollRange = (videoDuration || 6) * windowHeight

      // Video progress hesapla
      const videoProgress = Math.min(Math.max(scrollY / videoScrollRange, 0), 1)

      // Video zamanını hesapla
      targetVideoTime = videoProgress * (videoDuration || 6)
      targetVideoTime = Math.max(0, Math.min(targetVideoTime, videoDuration || 6))
    ...
    const checkLenis = () => {
      ...
      if (lenisInstance) {
        ...
        console.log('Hero: Lenis detected and attached')
      } else {
